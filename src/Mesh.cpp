...
Mesh::~Mesh()
{
    ASSERT( 0 == m_iVAO ,            "m_iVAO should be 0 (here it is '%d') - have you called destroy() ?.\n", m_iVAO );
    ASSERT( 0 == m_iEBO ,            "m_iEBO should be 0 (here it is '%d') - have you called destroy() ?.\n", m_iEBO );
    ASSERT( 0 == m_iVBOPosition ,    "m_iVBOPosition should be 0 (here it is '%d') - have you called destroy() ?.\n", m_iVBOPosition );
    ASSERT( 0 == m_iVBONormal ,      "m_iVBONormal should be 0 (here it is '%d') - have you called destroy() ?.\n", m_iVBONormal );
    ASSERT( 0 == m_iVBOTexCoords ,   "m_iVBOTexCoords should be 0 (here it is '%d') - have you called destroy() ?.\n", m_iVBOTexCoords );
}
//====================================================================================================================================
void Mesh::draw()
{
    //--------------------------------------------------------------------------------------------------------------------
    // precondition
    ASSERT( 0 != m_iVAO ,           "m_iVAO should not be 0 (here it is '%d')      - Did you call createFrom(...) ?.\n", m_iVAO );
    //--------------------------------------------------------------------------------------------------------------------
...
bool Mesh::createFrom( GLenum _eDrawMode, GLuint _iVertexCount, glm::vec3* _afPositions, glm::vec3* _afNormals, glm::vec2* _afTexCoords, GLuint _iElementsCount, GLuint* _auiIndices )
{
    ...
    ASSERT( 0 != _iVertexCount , "_iVertexCount should not be 0 (here it is '%d').\n", _iVertexCount );
    //--------------------------------------------------------------------------------------------------------------------
...
void Mesh::destroy()
{
    //--------------------------------------------------------------------------------------------------------------------
    // precondition
    ASSERT( 0 != m_iVAO ,            "m_iVAO should not be 0 (here it is '%d') - have you called createFrom(...) ?.\n", m_iVAO );
    //--------------------------------------------------------------------------------------------------------------------
