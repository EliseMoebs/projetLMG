...
{
    // init min with a HUGE positive value, and max with a HUGE negative value
    _rOutMin = glm::vec3( 10000, 10000, 10000 );
    _rOutMax = glm::vec3( -10000, -10000, -10000 );

    for( unsigned int i = 0; i < _aVertices.size(); i++ )
    {
        if( _aVertices[ i ].x < _rOutMin.x )
        {
            _rOutMin.x = _aVertices[ i ].x;
        }
        if( _aVertices[ i ].y < _rOutMin.y )
        {
            _rOutMin.y = _aVertices[ i ].y;
        }
        if( _aVertices[ i ].z < _rOutMin.z )
        {
            _rOutMin.z = _aVertices[ i ].z;
        }

        if( _rOutMax.x < _aVertices[ i ].x )
        {
            _rOutMax.x = _aVertices[ i ].x;
        }
        if( _rOutMax.y < _aVertices[ i ].y )
        {
            _rOutMax.y = _aVertices[ i ].y;
        }
        if( _rOutMax.z < _aVertices[ i ].z )
        {
            _rOutMax.z = _aVertices[ i ].z;
        }
    }
}
...
{
//    char* strAppliedLocale;
//    strAppliedLocale = setlocale( LC_NUMERIC, "us_US" );
//    LOG( "Applied locale '%s' \n", strAppliedLocale );

//    strAppliedLocale = setlocale(LC_NUMERIC, "C");
//    LOG( "Applied locale '%s' \n", strAppliedLocale );
