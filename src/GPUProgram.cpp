...
GPUProgram::~GPUProgram()
{
    ASSERT( 0 == m_iGPUProgramID ,           "m_iGPUProgramID should be 0 (here it is '%d') - have you called destroy() ?.\n", m_iGPUProgramID );
}
 //====================================================================================================================================
void GPUProgram::bind()
{
    //--------------------------------------------------------------------------------------------------------------------
    // precondition
    ASSERT( 0 != m_iGPUProgramID ,           "m_iGPUProgramID should not be 0 (here it is '%d')      - Did you call createFromFiles(...) ?.\n", m_iGPUProgramID );
    //--------------------------------------------------------------------------------------------------------------------
...
bool GPUProgram::createFromFiles( const std::string& _rstrVertexShaderPath, const std::string& _rstrGeometryShaderPath , const std::string& _rstrFragmentShaderPath )
{
    //--------------------------------------------------------------------------------------------------------------------
    // precondition
    ASSERT( "" != _rstrVertexShaderPath    , "_rstrVertexShaderPath should not be empty.\n", 0 );
    ASSERT( "" != _rstrFragmentShaderPath  , "_rstrFragmentShaderPath should not be empty.\n", 0 );
    ASSERT( 0 == m_iGPUProgramID ,           "m_iGPUProgramID should be 0 (here it is '%d') - have you already called this function ?.\n", m_iGPUProgramID );
    ASSERT( 0 == m_iGPUVertexShaderID ,      "m_iGPUVertexShaderID should be 0 (here it is '%d') - have you already called this function ?.\n", m_iGPUVertexShaderID );
    ASSERT( 0 == m_iGPUFragmentShaderID ,    "m_iGPUFragmentShaderID should be 0 (here it is '%d') - have you already called this function ?.\n", m_iGPUFragmentShaderID );
    ASSERT( 0 == m_iGPUGeometryShaderID ,    "m_iGPUGeometryShaderID should be 0 (here it is '%d') - have you already called this function ?.\n", m_iGPUGeometryShaderID );
    //--------------------------------------------------------------------------------------------------------------------
    ...
    //--------------------------------------------------------------------------------------------------------------------
    // postcondition
    ASSERT( 0 != m_iGPUProgramID ,           "m_iGPUProgramID should not be 0 (here it is '%d') - Did glCreateProgram(...) succeed ?.\n", m_iGPUProgramID );
    ASSERT( 0 == m_iGPUVertexShaderID ,      "m_iGPUVertexShaderID should be 0 (here it is '%d')     - Was destroyShader() called on this one ?.\n", m_iGPUVertexShaderID );
...
//        //--------------------------------------------------------------------------------------------------------------------
//        // precondition
//        ASSERT( 0 == _rOutShaderID ,    "_rOutShaderID should be 0 (here it is '%d') - have you already called this function ?.\n", _rOutShaderID );
//        //--------------------------------------------------------------------------------------------------------------------

    std::string strFragmentShader = readFileSrc( PATH_TO_DATA + _rstrShaderPath );
    ...
    printShaderCompileInfo( iShader, PATH_TO_DATA + _rstrShaderPath );

    //--------------------------------------------------------------------------------------------------------------------
    // iShader
    ASSERT( 0 != iShader ,    "iShader should not be 0 (here it is '%d') - Did glCreateShader(...) succeed ?.\n", iShader );
    //--------------------------------------------------------------------------------------------------------------------
...
void GPUProgram::destroyShader( GLuint& _rOutShaderID )
{
    //--------------------------------------------------------------------------------------------------------------------
    // precondition
    ASSERT( 0 != _rOutShaderID ,      "_rOutShaderID should not be 0 (here it is '%d') - Did you call createShader() ?.\n", _rOutShaderID );
    //--------------------------------------------------------------------------------------------------------------------
